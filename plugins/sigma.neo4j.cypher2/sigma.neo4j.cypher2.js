;(function (undefined) {
    ...
    sigma.neo4j.cypher_parse = function(result, producers) {
        ...
        // result.nodes.forEach(function (data) {
     ...
     */
    sigma.neo4j.cypher2 = function (neo4j, book, sig, callback, producers) {
        var endpoint = '/v1/graph',
            data, cypherCallback;
        ...
        cypherCallback = function (callback) {
            ...
            return function (response) {
                ...
                var nodeIdArr = sig.graph.nodes().map(function(node) {return node.id})
                var edgeIdArr = sig.graph.edges().map(function(edge) {return edge.id})

                var t0 = performance.now();

                graph.nodes.map(function(node) {
                    // if (nodeIdArr.findIndex(function(nodeId) {return parseInt(nodeId) === node.id}) === -1){
                    if (!nodeIdArr.includes((node.id))){
                        sig.graph.addNode(node);
                    }
                })

                graph.edges.map(function(edge) {
                    if (!edgeIdArr.includes((edge.id))){
                ...
                console.log("Call to doSomething3 took " + (t2 - t1) + " milliseconds.");
